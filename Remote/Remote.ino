...
void loop() {
	...
	byte		received_data[6],
				send_byte;												// Declare some global byte variables
	...
	for (byte i = 0; i < 6; i++)
		received_data[i] = Wire.read();       //Copy the bytes to the received_data array
	...
	// command the bot forward or to the right.  Sufficiently high values command the bot back or to the left.
	if (received_data[0] < NUNCHUCK_LOW_TRIGGER)
		send_byte |= BIT_1;						// x value is low, set bit 1 to command left
	if (received_data[0] > NUNCHUCK_HIGH_TRIGGER)
		send_byte |= BIT_2;						// x value is high, set bit 2 to command right
	if (received_data[1] < NUNCHUCK_LOW_TRIGGER)
		send_byte |= BIT_3;						// y value is low, set bit 3 to command forward
	if (received_data[1] > NUNCHUCK_HIGH_TRIGGER)
		send_byte |= BIT_4;						// y value is high, set bit 4 to command back
	if (send_byte)
